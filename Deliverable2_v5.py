...
LABEL_SMOOTHING   = 0.15   # CrossEntropyLoss label_smoothing factor
    ...
    for ep in range(n_epochs):
        ...
        if val_X_t is not None:
            ...
            if ep >= warmup_epochs:
                ...
                if not np.isnan(val_auc) and val_auc > best_auc + 0.01:
...
def train_eval_dl_fold(model_fn_factory, X_tr, y_tr, X_val, y_val, X_te, y_te,
    ...
    for _ in range(N_RANDOM_TRIALS):
        lr = 10 ** np.random.uniform(-4.0, -2.7)

        dropout = np.random.uniform(0.2, 0.7)

        wd = 10 ** np.random.uniform(-2.0, -0.7)

        dm = random.choice([16, 32, 48, 64])

        model = model_fn_factory(
            n_comp,
            dropout,
            dm,
            T_tr
    ...
    for combo in top_k:
        ...
        _, probs_val = _eval_dl(m, X_val_p)
        probs_val_list.append(probs_val)

    # Ensemble probabilities
    probs_te_avg  = np.mean(probs_te_list, axis=0)
    probs_val_avg = np.mean(probs_val_list, axis=0)

    # Find best threshold on validation set
    fpr, tpr, thresholds = roc_curve(y_val, probs_val_avg)
    ...
    y_pred_te = (probs_te_avg >= best_threshold).astype(int)
...
def main():
    ...
    filename_summary = "resnet_dl_improved_results_table_" + str(TRIM_SEC) + "_" + DL_INPUT_MODE + "_v3.csv"
    ...
    filename = "resnet_dl_improved_results_per_fold_" + str(TRIM_SEC) + "_" + DL_INPUT_MODE + "_v3.csv"
